...
class TestUnderstandEndpoints(unittest.TestCase):
    def test_analyse(self):
        ...
        assert cost["orion-fr@default"]["batch_size"] == 1, (
            f"`batch_size={cost['orion-fr@default']['batch_size']}` "
        ...
        assert outputs[0]["text"] == sentence, (
            f"`text` field in `outputs` does not match the" f" input sentence."
        )
        score, normalized_score = (
            outputs[0]["score"]["logprob"],
            outputs[0]["score"]["normalized_logprob"],
        )
        ...
        assert len(outputs) == len(sentence_list), (
            f"`len(outputs) = {len(outputs)}` despite "
        ...
        assert (
            outputs[0]["score"]["normalized_logprob"]
            > outputs[1]["score"]["normalized_logprob"]
        ), f"{message}"
        token_scores = [
            list(element.values())[0]
            for element in outputs[1]["score"]["token_logprobs"]
        ]
    ...
    def test_embed(self):
        ...
        assert (
            cost["orion-fr@default"]["batch_size"] == 1
        ...
        assert len(embedding) == 2048, (
            f"Shape of the embedding is {len(embedding)}!=2048 "
        ...
        assert len(first_embedding) == len(second_embedding), (
            f"Shape of the embeddings differ"
    ...
    def test_select(self):
        ...
        assert all(
            [math.isclose(ns, s / n)]
            for s, ns, n in zip(scores, normalized_scores, n_tokens)
        ), message
        assert all([s <= 0.0 for s in scores]), (
            f"Detected score > 0. This would give "
        ...
        assert best_score_with_conj > best_score_no_conj, (
            f"Conjunction `{conjunction}` does not" f"improve the score."
        )

        outputs, _, _ = selecter(reference, candidates, evaluate_reference=False)
        outputs_switched, _, _ = selecter(
            reference, candidates, evaluate_reference=True
        )
        score = outputs[0]["rankings"][0]["score"]["normalized_logprob"]
        score_switched = outputs_switched[0]["rankings"][0]["score"][
            "normalized_logprob"
        ]
        assert (
            score != score_switched
        ), f"Score of reference|candidate and candidate|reference are the same"

        # batching
        outputs, cost, rid = selecter([reference, reference], candidates)
        assert (
            len(outputs) == 2
        ), f"Output length is f{len(outputs)}, expected length 2."
        outputs, cost, rid = selecter([reference, reference], [candidates, [reference]])
        assert (
            len(outputs) == 2
        ), f"Output length is f{len(outputs)}, expected length 2."
        assert (
            len(outputs[0][0]["rankings"]) == 2
        ), f"Reference was scored against 2 candidates, got {len(outputs[0][0]['rankings'])} instead"
        assert (
            len(outputs[1][0]["rankings"]) == 1
        ), f"Reference was scored against 1 candidate, got {len(outputs[1][0]['rankings'])} instead"
    ...
    def test_compare(self):
        ...
        assert (
            cost["orion-fr@default"]["batch_size"] == len(candidates) + 1
        ...
        assert all([-1.0 <= cossim <= 1.0 for cossim in cos_sims]), (
            "Detected cosine similarity " "value outside of [-1, 1]."
        )
