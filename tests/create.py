...
class TestCreateEndpoint(unittest.TestCase):
    ...
    def test_control(self):
        # word bias
        creator = lightonmuse.Create("orion-fr")
        sentence = "La capitale de la France s'appelle"
        n_tokens = 20
        outputs, cost, rid = creator(sentence, n_tokens=n_tokens, seed=1)
        assert "Paris" in outputs[0]["completions"][0]["output_text"], "Paris is not in the output" \
                                                                       "for unbiased call."
        biases = {"Paris": -100., "Marseille": 3.5}
        frequency_penalty = 1.
